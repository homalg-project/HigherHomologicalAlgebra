...
AddLift( cat, 
   ...
   local P, N, M, A, B, l, basis_indices, Q, R_B, R_N, L_P, R_M, L_id_s, L_P_mod, morphism_1, morphism_2, 
    ...
    if IsZero( P ) then
      ...
      if sol = fail then
        ...
        GradedPresentationMorphism(
          Source( morphism1 ),
            DecideZeroRows( CertainColumns( sol, [1..NrRows(B) ] ), M ),
              Source( morphism2 ) );
    ...
    R_B := UnionOfRows(
      List( basis_indices,
        u -> KroneckerMat( Involution( Q * FRight( u, B ) ),
          HomalgIdentityMatrix( NrRows( A ), Q ) ) ) );
    
    if not IsZero( N ) then
      
      R_N := UnionOfRows( List( basis_indices,
        u-> KroneckerMat( Involution( Q * FRight( u, N ) ),
          HomalgIdentityMatrix( NrRows( A ), Q ) ) ) );
    ...
    L_P := UnionOfRows(
      List( basis_indices,
        u -> KroneckerMat( HomalgIdentityMatrix( NrColumns( M ), Q ), Q*FLeft( u, P ) ) ) );
    
    R_M := UnionOfRows(
      List( basis_indices,
        u -> KroneckerMat( Involution( Q * FRight( u, M ) ), HomalgIdentityMatrix( NrRows( P ), Q ) ) ) );
    
    L_id_s := UnionOfRows(
      List( basis_indices,
        u -> KroneckerMat( HomalgIdentityMatrix( NrRows( B ), Q ),
          Q * FLeft( u, HomalgIdentityMatrix( NrRows( A ), R ) ) ) ) );
    ...
    A_deco_list_vec := List( A_deco_list,
      mat -> UnionOfRows( List( [ 1..NrColumns( A ) ], i-> CertainColumns( mat, [ i ] ) ) ) );
    ...
    if not IsZero( N ) then
      ...
      mat1 := UnionOfColumns(
        [ R_B, R_N,
          HomalgZeroMatrix( NrRows( A ) * NrColumns( A ) * 2^l, NrRows( M ) * NrRows( P ) * 2^l, Q ) ]
          );
      
      mat2 := UnionOfColumns(
        [ L_P_mod,
          HomalgZeroMatrix( NrRows( P )*NrColumns( M )*2^l, NrRows( N )*NrColumns( P )*2^l, Q ), R_M ]
          );
    
    else
      
      mat1 := UnionOfColumns(
        R_B, HomalgZeroMatrix( NrRows( A )*NrColumns( A )*2^l, NrRows( M )*NrRows( P )*2^l, Q )
        );
    ...
    XX_ := UnionOfColumns(
      List( [ 1 .. v * 2^l ], i -> CertainRows( XX, [ ( i - 1 ) * s + 1 .. i * s ] ) ) );
    
    X_ := Sum( List( [ 1 .. 2^l ],
      i-> ( R * CertainColumns( XX_, [ ( i - 1 ) * v + 1 .. i * v ] ) ) *
        ring_element( basis_indices[ i ], R ) ) );
...
InstallMethod( GradedLeftPresentations,
    [ IsHomalgGradedRing ],
  function( K )
    local add_extra_methods, cat, FinalizeCategory, AddExtraMethods;
    
    if not HasIsExteriorRing( K ) or not IsExteriorRing( K ) then
    ...
    cat := GradedLeftPresentations( K : FinalizeCategory := false, AddExtraMethods := false );

  ...
  function( sigma, A )
    ...
    if HasIsOne( A ) and IsOne( A ) then
      ...
      return
        Iterated(
          [
            HomalgZeroMatrix( NrRows( A ), ( p - 1 ) * NrColumns( A ), A!.ring ),
            A,
            HomalgZeroMatrix( NrRows( A ), ( Length( basis_indices ) - p ) * NrColumns( A ), A!.ring )
  ...
  function( sigma, A )
    ...
    if HasIsOne( A ) and IsOne( A ) then
      ...
      return Iterated(
              ...
              [ HomalgZeroMatrix( ( p - 1) * NrRows( A ), NrColumns( A ), A!.ring ),
              
                A,
                
                HomalgZeroMatrix( ( Length( basis_indices ) - p ) * NrRows( A ), NrColumns( A ), A!.ring )
...
##
InstallMethodWithCache( FF2, [ IsList, IsHomalgMatrix, IsHomalgMatrix ],
...
##
DeclareOperationWithCache( "SolveTwoSidedEquationOverExteriorAlgebra",
...
InstallMethodWithCache( SolveTwoSidedEquationOverExteriorAlgebra,
  ...
  function( A, B, C )
    local C_deco, C_deco_list, C_deco_list_vec, C_vec, N, sol,
      Q, R, l, m, s, r, n, XX, YY, XX_, YY_, X_, Y_, basis_indices;
    ...
    C_deco_list_vec := List( C_deco_list, 
      c -> Iterated( List( [ 1 .. NrColumns( C ) ], i -> CertainColumns( c, [ i ] ) ), UnionOfRows ) );
    ...
    XX_ := Iterated( List( [ 1 .. s ],
      i -> CertainRows( XX, [ ( i - 1 ) * m * 2^l + 1 .. i * m * 2^l ] ) ), UnionOfColumns );
    
    YY_ := Iterated( List( [ 1 .. n * 2^l ],
      i -> CertainRows( YY, [ ( i - 1 ) * r + 1 .. i * r ] ) ), UnionOfColumns );
    
    X_ := Sum( List( [ 1 .. 2^l ],
      i -> ( R * CertainRows( XX_, [ ( i - 1 ) * m + 1 .. i * m ] ) ) *
        ring_element( basis_indices[ i ], R ) ) );
    
    Y_ := Sum( List( [ 1..2^l ],
      i-> ( R * CertainColumns( YY_, [ ( i - 1 )* n + 1 .. i * n ] ) ) *
        ring_element( basis_indices[ i ], R ) ) );

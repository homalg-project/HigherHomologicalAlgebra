...
##
InstallMethod( \/,
          [ IsGradedLeftPresentation, IsFreydCategory ],
          
  function( o, freyd_category )
    local S, degrees, range, m;
...
##
InstallMethod( \/,
          [ IsGradedLeftPresentationMorphism, IsFreydCategory ],
  function( alpha, freyd )
    local s, r;
    
    s := Source( alpha ) / freyd;
...
##
InstallMethod( \/,
          [ IsFreydCategoryObject, IsCapCategory ], # There is no filter for graded left presentations
          
  function( o, graded_fpres )
    local rm, degrees;
    
    if not IsBound( graded_fpres!.ring_for_representation_category ) then
...
##
InstallMethod( \/,
          [ IsFreydCategoryMorphism, IsCapCategory ], # There is no filter for graded left presentations
          
  function( alpha, graded_fpres )
    local s, r;
    ...
    s := Source( alpha ) / graded_fpres;
    
    r := Range( alpha ) / graded_fpres;
...
##
InstallMethod( \/,
          [ IsGradedRow, IsFreydCategory ],
  function( o, freyd_category )
    
    if not IsIdenticalObj( CapCategory( o ), UnderlyingCategory( freyd_category ) ) then
      
      TryNextMethod( );
      
    fi;
    
    return AsFreydCategoryObject( o );

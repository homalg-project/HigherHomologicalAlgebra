...
colift_lift_in_stable_category :=
    ...
    beta_deco := List( beta_deco, mat -> UnionOfRows( List( [ 1..NrColumns( beta ) ], i-> CertainColumns( mat, [ i ] ) ) ) );

    beta_vec := Q*UnionOfRows( beta_deco );

    gamma_deco := DecompositionOfHomalgMat( gamma );
   
    gamma_deco := List( gamma_deco, i-> i[ 2 ] );

    gamma_deco := List( gamma_deco, mat -> UnionOfRows( List( [ 1..NrColumns( gamma ) ], i-> CertainColumns( mat, [ i ] ) ) ) );

    gamma_vec := Q*UnionOfRows( gamma_deco );
    ...
    return PresentationMorphism( Range( alpha_ ), XX, Range( beta_ ) );
end;
...
all_colift_lift_in_stable_category :=
    ...
    for i in [ 1 .. NrColumns( sy_main_matrix ) ] do 
        ...
        Add( K, PresentationMorphism( Range( alpha_ ), a, Range( beta_ ) ) );
    od;

    return [ PresentationMorphism( Range( alpha_ ), XX, Range( beta_ ) ), K ];
end;

...
ab := LeftPresentations( ZZ );
Ho_ab := HomotopyCategory( ab );
m1 := 7;
m0 := 6;
n1 := 9;
n0 := 3;
...
C := HomotopyCategoryObject( [ c_1 ], 1 );
D := HomotopyCategoryObject( [ d_1 ], 1 );

phi_1 := PreCompose( c_1, h );
phi_0 := PreCompose( h, d_1 );

phi := HomotopyCategoryMorphism( C, D, [ phi_0, phi_1 ], 0 );

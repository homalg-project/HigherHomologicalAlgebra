...
#
# The test verifies:
#   (1) IsHomotopicToZeroMorphism correctly detects alpha and beta.

gap> START_TEST("test_homotopy_operations.tst");
...
gap> w := WitnessForBeingHomotopicToZeroMorphism( DirectSumFunctorial( [ alpha, beta ] ) );;
gap> MorphismMatrix( MorphismDatum( w[0] ) );;
gap> # @drop_example_in_Julia (until HomStructure is available in Adelman categories in Julia)

#
gap> STOP_TEST("test_homotopy_operations.tst", 1);

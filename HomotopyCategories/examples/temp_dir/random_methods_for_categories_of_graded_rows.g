ENHANCE_HOMALG_GRADED_RING_WITH_RANDOM_FUNCTIONS :=
...
InstallMethodWithCrispCache( MonomialsOfDegree,
  ...
  function( S, degree )
    ...
    #TODO Change this to use NConvex, since bugs in NormalizInterface are now fixed
...
DeclareOperation( "BasisOfExternalHomFromTensorUnitToGradedRow", [ IsGradedRow ] );

InstallMethodWithCrispCache( BasisOfExternalHomFromTensorUnitToGradedRow,
            [ IsGradedRow ],
function( M )
...
#
BasisOfExternalHomBetweenGradedRows :=
  function( a, b )
    ...
    mats := BasisOfExternalHomFromTensorUnitToGradedRow( hom_a_b );
...
CoefficientsOfMorphismOfGradedRowsWRTBasisOfExternalHom :=
  function( phi, basis )
    ...
    B := BasisOfExternalHomFromTensorUnitToGradedRow( hom_a_b );
